...
class AlienInvasion:
    ...
    def _update_bullets(self):
        ...
        for bullet in self.bullets.copy():
            if type(bullet) is Bullet and bullet.rect.bottom <= 0: #or bullet.rect.right <= settings.screen_width:
                self.bullets.remove(bullet)
            if type(bullet) is HBullet and bullet.rect.left > self.settings.screen_width:
                self.bullets.remove(bullet)
    ...
    def _fire_hbullet(self):
        """ Fire bullet if limit not reached yet """
        # Create a new bullet and add it to the bullets group
        if len(self.bullets) < self.settings.bullets_allowed:
            new_hbullet = HBullet(self)
    ...
    def _create_fleet(self):
        ...
        # Spacing between each alien is width of ine alien
        alien = Alien(self)
        number_aliens_x = self._get_number_aliens_x(alien)
        number_rows = self._get_number_rows(alien)
    ...
    def _get_number_aliens_x(self, alien):
        #Determine the number of aliens that fit in a row
        available_space_x = self.settings.screen_width - (2 * alien.rect.width)
        return available_space_x // (2 * alien.rect.width)

    def _get_number_rows(self, alien):
